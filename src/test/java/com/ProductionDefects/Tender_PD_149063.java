...
public class Tender_PD_149063 extends BaseClass_Web{
	...
	public void ProductionDefect_135540(String no) throws Throwable {
		...
		Dynamicity.updateDataIntoPropertyFile("TenderCreator", "HMEL_CREATOR", filePath_4);
		Dynamicity.updateDataIntoPropertyFile("TenderCreator_Password", "pass@123", filePath_4);
		Dynamicity.updateDataIntoPropertyFile("PRItemCount", "3", filePath_4);
		Dynamicity.updateDataIntoPropertyFile("RowCount", "All", filePath_4);
		...
		etenderPRcomponentobj.navigateToPRList();
			...
			ProductionDefectobj.deletecommertialTermsandContition();		
			etendercomponentobj.clickSubmitBtn_New();
			waitForObj(3000);
			String fSubmit = "//button[contains(text(),'Submit')]";
			boolean fielddisplayfSubmit = isElementDisplayed_Updated(By.xpath(fSubmit), 15);
			boolean interactablefSubmit = isElementEnable_Updated(By.xpath(fSubmit), 15);
			System.out.println("Field is displayed: " + fielddisplayfSubmit);
			System.out.println("Field is displayed: " + interactablefSubmit);
			if (fielddisplayfSubmit == true && interactablefSubmit == true) {
				universalNormalWait();
				waitForObj(10000);
				click(By.xpath(fSubmit), "final Submit");
			}
			ProductionDefectobj.validateAlertMSG();
