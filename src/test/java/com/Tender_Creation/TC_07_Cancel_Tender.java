...
public class TC_07_Cancel_Tender extends BaseClass_Web {
...
/*		etenderPRcomponentobj.commonLogin(Dynamicity.getDataFromPropertiesFile("IndentCreator", filePath_4), Dynamicity.getDataFromPropertiesFile("IndentCreator_Password", filePath_4), "Indent", "Creator");
	...
	*/		
		...
		etendercomponentobj.cancelTender();
		etendercomponentobj.AddTwoUsersForSequentialApproval_CancelWF();
		etendercomponentobj.enterTenderIdInSearch();
		ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));
	...
	//approving pending tender in 1st approver login	
		 rfqfromintendcomponentobj.ApproverLogin(pdfResultReport.testData.get("UserTenderApprover1"));
		  etendercomponentobj.GoToApprovalworkFlowPending_Cancel_TendersAndSearchTheTender();
	...
	//approving pending tender in 2nd approver login	
		rfqfromintendcomponentobj.ApproverLogin(pdfResultReport.testData.get("UserTenderApprover2"));
		  etendercomponentobj.GoToApprovalworkFlowPending_Cancel_TendersAndSearchTheTender();
			...
			mailAndAttachmentVeification(137 , 1,"Approvers marked in a open and restricted tender get notification regarding tender cancellation");
			mailAndAttachmentVeification(141 , 1,"Supplier marked in a open and restricted tender get notification regarding tender cancellation");
			mailAndAttachmentVeification(157 , 1,"Tender creator will get email notification after final approval of tender cancellation request.");
			ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));
			
		etenderPRcomponentobj.commonLogin(Dynamicity.getDataFromPropertiesFile("TenderCreator", filePath_4), Dynamicity.getDataFromPropertiesFile("IndentCreator_Password", filePath_4), "Tender", "Creator");
		etendercomponentobj.navigateToTenderListing();		
		 etendercomponentobj.enterTenderIdInSearch();
		 etendercomponentobj.checkTenderStatusforCancelledorDeleted("Cancelled");
		 ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));
