...
public class TC_05_Indent_Cancel_Indent extends BaseClass_Web {
	...
	public void CreateIndentDynamically(String no)
		...
		// cancel indent feature with approver
		etenderPRcomponentobj.commonLogin(Dynamicity.getDataFromPropertiesFile("IndentCreator", filePath_4), Dynamicity.getDataFromPropertiesFile("IndentCreator_Password", filePath_4), "Indent", "Creator");
		rfqfromintendcomponentobj.navigateToIndentListing();
		rfqfromintendcomponentobj.enterIndentNoForSearch();
		rfqfromintendcomponentobj.validateCancelIndentFeature();
		rfqfromintendcomponentobj.AddSingleUsersForSequentialApproval_IndentWF_Predefined( pdfResultReport.testData.get("UserTenderApprover1"));
		ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));
		...
		etendercomponentobj.tenderApprovalDecision("final Cancel Indent"); 
		 mailAndAttachmentVeification(168 , 1,"Approver(s) will get an intimation mail after document is cancelled by the creator");
		ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));				 
