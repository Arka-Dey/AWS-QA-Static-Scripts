...
public class PO_02_Create_PO_From_SN_Multiple_Approver extends BaseClass_Web {
	...
	public void createPOFromSN_Dynamically(String no) throws Throwable {
		...
		posttendercomponentobj.CreatePOfromSN(Dynamicity.getDataFromPropertiesFile("CompletedSN", filePath_4),
				Dynamicity.getDataFromPropertiesFile("poTemplate", filePath_4));
		...
		posttendercomponentobj.provide_PO_ApproverComment();
		posttendercomponentobj.purchaseOrderEvaluationDecision("approve");
		mailAndAttachmentVeification(222, 1,
				"Coordinator will get an intimation mail after min number of approval required is reached for Purchase Order");
		ProductionDefectobj.Logout(Dynamicity.getDataFromPropertiesFile("Environment", filePath_4));
		...
		etenderPRcomponentobj.commonLogin(Dynamicity.getDataFromPropertiesFile("poCreator", filePath_4),
				Dynamicity.getDataFromPropertiesFile("poCreator_password", filePath_4), "PO", "Creator");
		waitForObj(5000); // static wait due to status not updated quick
